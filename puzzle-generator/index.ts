...
const rng = seedrandom("zachpwn3");

// 1. RDDDRURRRDLLDLDRRURRDDDR (clrtffxpry <-> pctf{l1ke_)
// 2. RDDRURDDDRURULURRDDDDDRD (nyghq7xksg <-> a_lost_ag3)
// 3. DRDDDDRUURRRULURRDDDDDDR
...
let tryMany = () => {
    ...
    for (let i = 0; i < 100000; i++) {
        let game = Solver.newGame(width);
        let results = Solver.solve(game, false);
        if (results.length === 1 && Solver.getPathLen(results[0]) >= (width + 4) * 2) {
            Solver.drawHistory(results[0]);
            process.exit(0);
        }
